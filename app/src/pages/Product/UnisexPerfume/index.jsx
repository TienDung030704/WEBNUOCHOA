...
function UnisexPerfumePage() {
  const navigate = useNavigate();

  const { displayProducts } = useProductFilter("UNISEX");
  ...
  return (
    <div className="catalog-page-enter min-h-screen bg-[#1b1b1b] text-white">
      <section className="relative overflow-hidden border-b border-white/8 bg-[#121212] pt-[132px] pb-16">
        ...
        <div className="hero-gradient-expand absolute inset-y-0 left-0 w-[36%] bg-gradient-to-r from-[#2d4f4f]/58 via-black/20 to-transparent" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(108,173,173,0.16),transparent_38%)]" />

        <div className="reveal-up relative z-10 mx-auto flex w-full max-w-[1280px] flex-col items-center px-[22px] text-center">
          <h1 className="text-[46px] font-medium tracking-[-0.04em] text-white md:text-[64px]">
            Nước hoa unisex
          </h1>

          <div className="mt-5 flex items-center gap-3 text-sm text-white/68 md:text-[15px]">
            <span>Trang chủ</span>
            <ChevronRight size={15} className="text-white/35" />
            <span>Nước hoa</span>
            <ChevronRight size={15} className="text-white/35" />
            <span className="text-white">Nước hoa unisex</span>
          </div>
        </div>
      </section>

      <section className="relative bg-[#202020] py-14">
        <div className="mx-auto grid w-full max-w-[1280px] grid-cols-1 gap-8 px-[22px] lg:grid-cols-[290px_minmax(0,1fr)]">
          <PerfumeCategorySidebar />

          <div className="space-y-6">
            <div className="reveal-up reveal-delay-2 flex items-center gap-4">
